...
class InputWaiter {
    ...
    For loading Liber Primus Pages for Cicada 3301 
    Pages are loaded with runes being represented by their corresponding 'Index' (e.g. F = 0, U = 1)
    Apply text transliteration to convert Index to English or Runic
    */
    loadLP(e) {      
      e.preventDefault();
        const filename = window.prompt(`
          For loading Liber Primus Pages for Cicada 3301 
            Pages are loaded with runes being represented by their corresponding 'Index' (e.g. F = 0, U = 1)
            Apply text transliteration to convert Index to English or Runic
            Enter a page number or a range of numbers to load LP text into CyberChef(or 100-116 for LP1). \n Add 'Cicada 3301 > Text Transliteration'; Input: Index/Dash.`
            , "0-2, 57");
        ...
        if (filename) {
            ...
            ranges = ranges.concat(findRanges);
            console.log ("filenames: " + filenames + " Ranges :" + findRanges);
            if (ranges.length > 0){
                ranges.forEach(function(range){
                    filenames.splice(filenames.indexOf(range), 1); // remove the range from the complete list of filenames
                    let bounds = range.split("-");
                    console.log ("filenames: " + filenames + " bounds :" + bounds);
                    let lowerbound = bounds[0];
                    let upperbound = bounds[1];
                    for (let i = lowerbound; i <= upperbound; i++){
                        filenames.push("" + i);//push all appropriate pages
            ...
            for (let i = 0; i <  filenames.length; i++) {
                ...
                if (LiberPages[pageNumber]){
                  ...
                  //  cipherText += "\n"; //Add line break at end of the page
